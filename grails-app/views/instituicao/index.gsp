		...
		<div id="list-instituicao" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="cnpj" title="${message(code: 'instituicao.cnpj.label', default: 'Cnpj')}" />
					
						<g:sortableColumn property="descricao" title="${message(code: 'instituicao.descricao.label', default: 'Descricao')}" />
				...
				<g:each in="${instituicaoInstanceList}" status="i" var="instituicaoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${instituicaoInstance.id}">${fieldValue(bean: instituicaoInstance, field: "cnpj")}</g:link></td>
					
						<td>${fieldValue(bean: instituicaoInstance, field: "descricao")}</td>
