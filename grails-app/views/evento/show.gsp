...
<body>
	<a href="#show-evento" class="skip" tabindex="-1"><g:message
		...
		<ol class="property-list evento">
			<g:if test="${eventoInstance?.nome}">
				<li class="fieldcontain"><span id="nome-label"
					class="property-label"><g:message code="evento.nome.label"
							default="Nome" /></span> <span class="property-value"
					aria-labelledby="nome-label"><g:fieldValue
							bean="${eventoInstance}" field="nome" /></span></li>
			</g:if>
			...
			<g:if test="${eventoInstance?.locais}">
				<li class="fieldcontain"><span id="locais-label"
					class="property-label"><g:message code="evento.locais.label"
							default="Locais" /></span> <g:each in="${eventoInstance.locais}"
						var="l">
						<span class="property-value" aria-labelledby="locais-label"><g:link
								controller="localDoacao" action="show" id="${l.id}">
								${l?.encodeAsHTML()}
							</g:link></span>
					</g:each></li>
			</g:if>
			...
			<g:if test="${eventoInstance?.necessidades}">
				<li class="fieldcontain"><span id="necessidades-label"
					class="property-label"><g:message
							code="evento.necessidades.label" default="Necessidades" /></span> <g:each
						in="${eventoInstance.necessidades}" var="n">
						<span class="property-value" aria-labelledby="necessidades-label"><g:link
								controller="itemNecessidade" action="show" id="${n.id}">
								${n?.encodeAsHTML()}
							</g:link></span>
					</g:each></li>
			</g:if>
			<g:if test="${eventoInstance?.author}">
				<li class="fieldcontain"><span id="author-label"
					...
					aria-labelledby="author-label"><g:link
							...
							${eventoInstance?.author?.encodeAsHTML()}
						</g:link></span></li>
			</g:if>
