...
<body>
	<a href="#show-localDoacao" class="skip" tabindex="-1"><g:message
		...
		<ol class="property-list localDoacao">
			...
			<g:if test="${localDoacaoInstance?.evento}">
				<li class="fieldcontain"><span id="evento-label"
					class="property-label"><g:message
							code="localDoacao.evento.label" default="Evento" /></span> <span
					class="property-value" aria-labelledby="evento-label"><g:link
							controller="evento" action="show"
							id="${localDoacaoInstance?.evento?.id}">
							${localDoacaoInstance?.evento?.encodeAsHTML()}
						</g:link></span></li>
			</g:if>

		</ol>
		<g:form url="[resource:localDoacaoInstance, action:'delete']"
			method="DELETE">
